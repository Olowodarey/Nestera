...
describe('StellarWebhookController', () => {
    ...
    describe('handleWebhook', () => {
        ...
        it('should return 200 and success status for valid signature', async () => {
            const result = await controller.handleWebhook(mockPayload, validSignature);
            expect(result).toEqual({ status: 'success' });
        });

        it('should throw UnauthorizedException for missing signature', async () => {
            await expect(controller.handleWebhook(mockPayload, undefined)).rejects.toThrow(
        ...
        it('should handle webhook with different body order', async () => {
            ...
            const result = await controller.handleWebhook(reorderedPayload, validSignature);
            expect(result).toEqual({ status: 'success' });
        });
    });
});
